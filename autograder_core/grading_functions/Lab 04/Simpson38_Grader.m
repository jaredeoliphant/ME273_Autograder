...
% DATE: 2/6/2018
% 
% PURPOSE: Function for grading Lab 4, part II: Simpson's 3/8 integration - but only
% for nsegs which are evenly divisible by three.
%
% INPUTS: 
...
ErrorMessage = '';
    ...
    try
        ...
        Solution = 2.002009846628558;   % Obtained by I = Simpson13_2087(x,y)
        ...
        Feedback = ['Score 0 because code failed. MATLAB Error Message: ',ERR.message];     % Send them an explanation and the MATLAB error message. 
    ...
    try
        ...
        x = linspace(0,pi,6);
        y = sin(x);    % A sine function from 0 to pi in 10 increment (11 data points).
        Solution = 2.002009846628558;   % Obtained by I = Simpson38_2087(x,y)
        ...
        % student gets a penalty and a feedback message.        score = (1 - abs((I-Solution)/Solution))*0.8;
        score = (1 - abs((I-Solution)/Solution))*0.80;      % 20% penalty for flawed implementation.
        Feedback = ['When 5 segments were used, your code produced an integral of ',num2str(I), ' - it should have produced an error message instead.' ];
